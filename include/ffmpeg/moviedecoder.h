...
class MovieDecoder {
  public:
	explicit MovieDecoder( const std::string &filename, AVPixelFormat format = AV_PIX_FMT_YUV420P );
	~MovieDecoder();
  ...
  private:
	...
	//
	void createFrame( AVFrame **avFrame, int width, int height, AVPixelFormat format ) const;

	bool initializeVideo();
	bool initializeAudio();

	bool decodeVideoPacket( AVPacket &packet );
	void convertVideoFrame( AVPixelFormat target ) const;
  ...
  private:
	...
	std::unique_ptr<std::thread> m_pPacketReaderThread;
	bool                         m_bInitialized;
	bool                         m_bHasVideo;
	bool                         m_bHasAudio;
	bool                         m_bPlaying;
